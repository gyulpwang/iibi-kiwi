					...
					<select name="up" id="up" onchange="showDW(this.value)">
						<optgroup label="상위분류 선택">
							...
							<c:forEach var="ul" items="${upList}" varStatus="st">
								<option id="uo${ul.upcode}" value="${ul.upcode}">${ul.upname}</option>
							</c:forEach>
						</optgroup>
					</select>
				</c:if>
...
<!-- 카테고리 별 상품 보러가기 폼 시작 ------------- -->
<form name="cf" id="cf" action="<c:url value='/talent/talentCate.do'/>">
	<input type="hidden" name="dwCode" id="dwCode">
	<input type="hidden" name="dwName" id="dwName">
</form>
<!-- ----------------------------------------------- -->
<!-- 전체보기 목록으로 가기 폼 --------------------- -->
...
<script type="text/javascript">
	var showDW=function(upcode){
		...
		$.ajax({
			type:'GET',
			url:'${pageContext.request.contextPath}/talent/dwCategory.do?upcode='+upcode,
			dataType:'json',
			success:function(res){
				...
				$.each(res, function(data){
					str+="<tr><td colspan='2'>";
					str+="<a class='clrBlack' href='#' onclick='showTalent("+this.dwcode+",\""+this.dwname+"\")'>";
					str+=this.dwname;
					str+="</a>";
					str+="</td></tr>";
				});
...
<script type="text/javascript">
	var showTalent=function(dcode, dname){
		$('#dwCode').val(dcode);
		$('#dwName').val(dname);
		$('#cf').submit();
	}
